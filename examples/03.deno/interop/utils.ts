import { parse, stringify } from "https://deno.land/std@0.102.0/encoding/yaml.ts";
...
export function traverse<T extends KubeResource>(data: KubeResource | KubeResource[], predicate: predicate<KubeResource>, fn: (t:T)=> void){
...
function createConfigurationContainer(data: KubeResource[]){
    return {
        traverse<T extends KubeResource = KubeResource>
                (x: predicate<KubeResource,T>, fn:(x:T)=> void  ){
...
export function importResources(yaml:string){
    const data = yaml.split(/(^|\n)-{3,}(\n|$)/g).filter(x=>x.trim() !== "").map(x=>parse(x)) as KubeResource[] | KubeResource;
